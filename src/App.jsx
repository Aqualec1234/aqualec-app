...
import { CalendarDays, ClipboardList, Users, HardHat, Plus, ChevronLeft, ChevronRight, X, Camera, ArrowLeft, LogOut, PoundSterling } from "lucide-react";
...
function jobFromDb(row) {
  return {
    ...
    department: row.department || "plumbing",
  ...
  const createJob = async (form) => {
    ...
    try {
      ...
      const { data: newJob, error: jobErr } = await supabase.from("jobs").insert({
        ...
        job_type: form.jobType, priority: form.priority, department: form.department || "plumbing", customer_id: customerId,
...
function JobModal({ modal, users, customers, currentUserId, onClose, onCreate }) {
  ...
  const handleSubmit = () => {
    if (customerMode === "existing" && selectedCustomer) {
      onCreate({ kind, customerId: selectedCustomer.id, customer: selectedCustomer.name, address: selectedCustomer.address, phone: selectedCustomer.phone, description, assignedTo, jobType, priority, department });
    } else {
      onCreate({ kind, customerId: null, customer: customer.trim(), address, phone, description, assignedTo, jobType, priority, department });
    }
  };
